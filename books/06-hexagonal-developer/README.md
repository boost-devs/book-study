...
- `샐리`: 코드를 짤 때나 문서를 작성할 때 항상 이런 마음가짐을 가지고 써야겠다고 생각했다.
...
- `펭귄`: 문장이 글을 적는 곳의 너비를 넘어가면 문장의 호흡이 길어진다고 판단하고 글을 잘게 자르는 편이다.
- `샐리`: 요즘은 다들 메신저를 쓰다보니 긴 문장에 대해 신경을 못쓰는 경우가 많은 것 같다. 메일을 쓸 때 이런 습관이 독이 되는 것 같다.
- `튜브`: 문장을 자르다 보면 글 내용이 정리되는 경우도 있는 것 같다.
...
- `샐리`: 어휘력을 늘릴 수 있는 책을 좀 읽어야겠다는 생각을 한다…
- `펭귄`: 관심사(ex. 방탈출, 커피)에서 시작하거나 간단한 후기 쓰는 것부터 시작하면 조금 시작하기 수월한 것 같다. 물론 에디터 키는 것조차 귀찮아서 문제지만…
...
![boss vs leader](./img/boss-vs-leader.png)

> <strong><i>p256. 주의할 점은 내가 제시한 아이디어가 최고라는 자만에 빠지지 않는 것이다.</i></strong> 🍋

- `샐리`: 혼자서만 일을 한다면 내 아이디어가 최고라는 생각에 빠지기 쉽다. 리뷰하고 협업하는 것이 이런 함정에서 빠져나올 수 있는 방법이지 않을까?

> <strong><i>p258. 변화가 필요하다면 사람이 아닌 프로세스와 시스템에 집중하자.</i></strong> 🌵🍋🐧

- `펭귄`: 사람이 아닌 프로세스와 시스템을 집중할 수 있다는 것 자체가 리더가 아닐까? 그런 사람을 본적이 있는데 정말 경의로웠다.

> <strong><i>p259. 주니어를 벗어나야 할 개발자가 이런 다양한 역량을 성장시키는 데 소홀하고 구현 기술만 파고 있으면 오히려 문제가 될 수 있다.</i></strong> 🍋

- `샐리`: 스터디를 했기 때문에 개발자가 키워야할 다양한 역량에 대해 배울 수 있었던 것 같다. 포기하지 말고 얇고 길게 갑시다!

> <strong><i>p260. 직원의 성장을 바란다면 일을 대신하지 말고 마음의 여유를 갖자. 위기 순간에는 바른 조치를 위해 직접 나서야 할 때도 있지만 위험한 상황이 아니면 대신하고 싶다면 유혹을 견뎌야 한다.</i></strong> 🐧🌵
...
- `샐리`: 마이크로매니징을 하게되면 딱 시키는 일까지만 하게 되더라.
- `펭귄`: 마이크로매니징을 받는 입장, 하는 입장 모두 되어봤는데 많이 반성하게 되었다.
...
> <strong><i>p264. 반대로 리더가 조직 내에서 성과를 내지 못하면 팔로워 역시 성과를 내기 어렵다. 리더와 팔로워는 공생 관계이다.</i></strong> 🐧
...
- `샐리`: 좋은 코드를 만들기 위해서 모인거니, 코드를 지적당한다고 해서 기분 나빠할 필요는 없다!
- `펭귄`: 같이 협업하는 사람이기에 서로 기분 상하지 않도록 쿠션어가 필요하고 이 코드가 이상하다면 그에 대한 충분한 근거가 있어야 된다고 생각한다. 결국 같이 만들어가는 코드이기 때문에 더 좋은 코드를 만들기 위한 의견을 주고 받는 것은 매우 좋지만 단순히 “이거 별로에요"는 싸워야 된다.
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>스터디하기에 가장 좋았던 책이었다. 개발 뿐만 아니라 업무, 협업 그리고 리더까지 다양한 내용이 나와서 이야기를 나누기가 정말 좋았다. 책에서 중간 중간에 저자의 경험을 적어놓은 부분이 있었는데, 3년 가까이 일하면서 내가 겪었던 일과 비슷한 일이 있었어서 많이 공감되었다. 특히 11장은 많은 깨달음을 얻을 수 있었고 나중에 내가 리더가 된다면 다시 읽어보면 좋을 것 같다.</td>
    </tr>
    <tr>
