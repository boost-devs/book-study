...
- `샐리` : ‘사용자가 개발자의 의도에 맞지 않게 쓰지 않도록 하기 위해 생겨난 것 같다’고 느낀 부분이다.
...
- `펭귄`: Python 쓰다가 요새 Java를 쓰고 있는데 타입 오류가 날 때마다 힘들었는데 “타입 검사기가 좀 빡빡할 뿐이구나"라고 생각하니 언어가 좀 더 이해가 됐다! 이 책은 두고두고 봐야할 듯!
...
- `펭귄`: 같이 정리한 불변/공변/반변성!
  ...
  전재:
  - Student -> Person
  - GenericClass<T> 제네릭 클래스

  문제:
  - GenericClass<Student>과 GenericClass<Person>의 상속 관계는?

  정답:
  - 상속 관계가 상관이 없음 = 불변(Invariance)
  - 상속 관계가 그대로 감 = 공변(Covariance) 👉 GenericClass<Student> -> GenericClass<Person>
  - 상속 관계가 반대로 됨 = 반변(Contravariance) 👉 GenericClass<Person> -> GenericClass<Student>
  ```
...
> <strong><i>p310. 이렇게 정적 타입을 바탕으로 함수를 선택하는 것을 정적 선택(static dispatch)이라 부른다.</strong></i> 🐧
...
> <strong><i>p342. 특정 타입을 어떤 타입클래스에 속하게 만들고 싶다면 타입클래스 인스턴스(typeclass instance)를 정의해야 한다.</strong></i> 🐧🍋

> <strong><i>p351. 이처럼 추상 클래스로는 제네릭 타입이 타입 인자에 상관없이 항상 만족하는 성질만 표현할 수 있다. 하지만 타입클래스로는 항상 만족하는 성질은 물론이고 특정 타입 인자를 받은 경우에만 만족하는 성질 역시 표현할 수 있다.</strong></i> 🐧🍋

- `펭귄`: 실제로 사용한 걸 본 적이 없어서 낯설다…

> <strong><i>p362. 카인드는 곧 타입의 타입인 셈이다.</strong></i> 🐧🍋

> <strong><i>p345. 함수는 아무 때나 정의할 수 있지만 메서드는 클래스를 정의할 때만 정의할 수 있다.</strong></i> 🌵🍋
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>Python을 계속 쓰다가 최근에 JVM 계얼의 언어(ex. Java, Scala)를 사용하게 되었는데 이 책이 정말 큰 도움을 주었다. 개인적으로 정적 언어보다는 동적 언어를 선호했지만, 이 책을 통해 "왜 정적 언어를 사용하는지", "정적 언어의 체계는 어떻게 되었는지" 알 수 있었어서 좋았고 특히 "공변", "반변", "불변"을 완벽히 이해하게 되었다. 정적 언어를 100% 마스터하지 않는 이상은 이 책을 두고두고 볼 것 같다.</td>
    </tr>
    <tr>
