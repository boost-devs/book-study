...
> <strong><i>p.157 코드에 유연성, 유지보수성, 재사용성을 제공하는 버팀목이 바로 단위테스트다. 이유는 단순하다. 테스트 케이스가 있으면 변경이 두렵지 않으니까!</i></strong> 🐧

> <strong><i>p.157 테스트 케이스가 없다면 모든 변경이 잠정적인 버그다.</i></strong> 🍋
