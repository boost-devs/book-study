...
> <strong><i>p.228 동시성은 '때로' 성능을 높여준다. </i></strong> 🌵🙂🐧

> <strong><i>p.228 일반적으로 동시성 버그는 재현하기 어렵다. 그래서 진짜 결함으로 간주되지 않고 일회성 문제로 여겨 무시하기 쉽다. </i></strong> 🍋🌵🙂🐧

> <strong><i>p.230 동시성 코드는 다른 코드와 분리하라. </i></strong> 🍋🌵

> <strong><i>p.231 객체를 복사해 읽기 전용으로 사용하는 방법이 가능하다. 어떤 경우에는 각 스레드가 객체를 복사해 사용한 후 한 스레드가 해당 사본에서 결과를 가져오는 방법도 가능하다. </i></strong> 🍋

- `샐리`: 어떻게 사본이 lock을 대신할 수 있을까?

> <strong><i>p.232 독자적인 스레드로, 가능하다면 다른 프로세서에서, 돌려도 괜찮도록 자료를 독립적인 단위로 분할하라. </i></strong> 🍋🌵

- `샐리`: 큰 단위의 자료를 작은 단위로 잘라서 공유자원의 공유 범위를 줄이라는 말일까?

> <strong><i>p.232 일부 클래스 라이브러리는 스레드에 안전하지 못한다. </i></strong> 🙂

> <strong><i>p.234 처리율을 강조하면 기아 현상이 생기거나 오래된 정보가 쌓인다. 갱신을 허용하면 처리율에 영향을 미친다. </i></strong> 🍋

- `샐리`: 여기서 처리율의 의미는 한번에 처리하는 일의 크기일까?

> <strong><i>p.234 대개는 쓰기 스레드가 버퍼를 오랫동안 점유하는 바람에 여러 읽기 스레드가 버퍼를 기다리느라 처리율이 떨어진다. </i></strong> 🍋

> <strong><i>p.235 주의해서 설계하지 않으면 데드락, 라이브락, 처리율 저하, 효율성 저하 등을 겪는다. </i></strong> 🍋

> <strong><i>p.235 공유 객체 하나에는 메서드 하나만 사용하라. </i></strong> 🍋🐧

> <strong><i>p.236 코드를 짤 때는 임계영역 수를 최대한 줄여야 한다. ... 필요 이상으로 임계영역 크기를 키우면 스레드 간에 경쟁이 늘어나고 프로그램 성능이 떨어진다. </i></strong> 🍋

> <strong><i>p.236 영구적으로 돌아가는 시스템을 구현하는 방법과 잠시 돌다 깔끔하게 돌다 깔끔하게 종료하는 시스템을 구현하는 방법은 다르다. </i></strong> 🍋🌵

> <strong><i>p.237 코드가 올바르다고 증명하기는 현실적으로 불가능하다. 테스트가 정확성을 보장하지는 않는다. </i></strong> 🍋

> <strong><i>p.237 테스트가 실패하면 원인을 추적하라. 다시 돌렸더니 통과하더라는 이유로 그냥 넘어가면 절대로 안 된다. </i></strong> 🍋

> <strong><i>p.238 일회성 문제란 존재하지 않는다고 가정하는 편이 안전하다. '일회성' 문제를 계속 무시한다면 잘못된 코드 위에 코드가 계속 쌓인다. </i></strong> 🍋

> <strong><i>p.238 다른 플랫폼에서 돌려봐라 </i></strong> 🙂

> <strong><i>p.238 스레드 환경 밖에서 생기는 버그와 스레드 환경에서 생기는 버그를 동시에 디버깅하지 마라. 먼저 스레드 환경 밖에서 코드를 올바로 돌려라. </i></strong> 🍋🌵🙂🐧

> <strong><i>p.239 스와핑을 일으키려면 프로세서 수보다 많은 스레드를 돌린다. 스와핑이 잦을수록 임계영역을 빼먹은 코드나 데드락을 일으키는 코드를 찾기 쉬워진다. </i></strong> 🍋

> <strong><i>p.240 다중 스레드 코드는 플랫폼에 따라 다르게 돌아간다. 따라서 코드가 돌아갈 가능성이 있는 플랫폼 전부에서 테스트를 수행해야 마땅하다. </i></strong> 🍋

> <strong><i>p.241 yield()를 삽입하면 코드가 실행되는 경로가 바뀐다. </i></strong> 🍋🙂

> <strong><i>p.243 클라이언트에게 공유 상태를 관리하는 책임을 떠넘기지 않는다. 필요하다면 객체 설계를 변경해 클라이언트에게 편의를 제공한다. </i></strong> 🍋🐧
