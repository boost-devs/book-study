...
> **p118-p119** 어째서 수많은 프로그래머가 조회(get)함수와 설정(set)함수를 당연하게 공개(public)해 비공개 변수를 외부에 노출할까? </br>
...
> **p139** (위 코드에서) 실상은 null 확인이 너무 많아 문제다. 🌵

> **p140** null을 반환하기 보단 빈 리스트를 리턴하자. 🙂🌵

> **p142** 대다수 프로그래밍 언어는 호출자가 실수로 넘기는 null을 적절히 처리하는 방법이 없다. 그렇다면 애초에 null을 넘기지 못하도록 금지하는 정책이 합리적이다. 🌵
