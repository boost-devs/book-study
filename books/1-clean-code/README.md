<div align="center">
  ...
  <div>
    ...
    <img src="https://img.shields.io/badge/%EA%B8%B0%EA%B0%84-2022.02.15%20~%202022.07.26-52b788?style=for-the-badge"/>
  </div>
</div>
...
깨끗한 코드는 읽기도 좋야야 하지만 안정성도 높아야 한다.
...
> <strong><i>p194. 도시가 돌아가는 또 다른 이유는 적절한 추상화와 모듈화 때문이다. 그래서 큰 그림을 이해하지 못할지라도 개인과 개인이 관리하는 ‘구성요소’는 효율적으로 돌아간다. </i></strong> 🍋🌵🐧

- `튜브`: 건축과의 비유가 뒤에도 이어지는데 적절한 비유인듯!

> <strong><i>p195. 소프트웨어 시스템은 (애플리케이션 객체를 제작하고 의존성을 서로 ‘연결’하는) 준비 과정과 (준비 과정 이후에 이어지는) 런타임 로직을 분리해야 한다. </i></strong> 🍋

- `펭귄`: 서버 개발할 때 보면, DB세션을 생성하는 과정과 실제 요청을 처리하는 로직이 분리되어 있는데 그게 생각이 났다.
- `샐리`: 함수를 분리하는 기준을 조금 더 명확하게 세울 수 있게 되었다. 시작과정의 초기화와 런타임 중 오류를 처리하는 초기화는 다르듯이!

> <strong><i>p195. 초기화 지연(Lazy Initialization)혹은 계산 지연(Lazy Evaluation)이라는 기법이다. ... 책임이 둘이라는 말은 메서드가 작업을 두 가지 이상 수행한다는 의미다. 즉, 작게나마 단일 책임 원칙(Single Responsibility Principle, SRP)을 깬다는 말이다. </i></strong> 🍋🌵

- `펭귄`: [단일 책임 원칙](https://steady-coding.tistory.com/370)이 잘 정리된 블로그

> <strong><i>p196 - p197. 시스템 생성과 시스템 사용을 분리하는 한 가지 방법으로, 생성과 관련한 코드는 모두 main이나 main이 호출하는 모듈로 옮기고, 나머지 시스템은 모든 객체가 생성되었고 모든 의존성이 연결되었다고 가정한다. … main 함수에서 시스템에 필요한 객체를 생성한 후 이를 애플리케이션에 넘긴다. … 모든 화살표가 main 쪽에서 애플리케이션 쪽을 향한다. 즉, 애플리케이션은 main이나 객체가 생성되는 과정을 전혀 모른다는 뜻이다. 단지 모든 객체가 적절히 생성되었다고 가정한다. </i></strong> 🍋🙂🐧🌵

- `후미`: main과 다른 기능들을 분리하자. 설정과, 분리하는 부분(코드)를 분리

> <strong><i>p197. LineItem을 생성하는 시점은 애플리케이션이 결정하지만 LineItem을 생성하는 코드는 애플리케이션이 모른다. </i></strong> 🍋🌵🐧

- `펭귄`: “팩토리 메서드 패턴 vs 추상화 팩토리 패턴” 관련해서 찾아보시면 좋을 것 같아요!

> <strong><i>p198. 사용과 제작을 분리하는 강력한 메커니즘 하나가 의존성 주입(Dependency Injection, DI)이다. 의존성 주입은 제어 역전(Inversion of Control, IoC) 기법을 의존성 관리에 적용한 메커니즘이다. 제어 역전에서는 한 객체가 맡은 보조 책임을 새로운 객체에게 전적으로 떠넘긴다. </i></strong> 🍋🌵

- `튜브`: [의존성 주입](https://kotlinworld.com/64)
- `펭귄`: 이 장을 보는 시각을 바꿔준 문장. 여기서부터 뒤에 내용을 100% 이해하려면 자바 스프링 부트를 공부해야 한다.

> <strong><i>p198. 진정한 의존성 주입은 여기서 한 걸음 더 나간다. 클래스가 의존성을 해결하려 시도하지 않는다. 클래스는 완전히 수동적이다. 대신에 의존성을 주입하는 방법으로 설정자(setter) 메서드나 생성자 인수를 제공한다. … 대다수 DI 컨테이너는 필요할 때까지는 객체를 생성하지 않고, 대부분은 계산 지연이나 비슷한 최적화에 쓸 수 있도록 팩토리를 호출하거나 프록시를 생성하는 방법을 제공한다. </i></strong> 🍋

- `샐리`: 의존성 주입과 계산지연과 같은 최적화는 무슨 관계가 있을까?

> <strong><i>p199. 테스트 주도 개발(Test-driven Development, TDD), 리팩터링, 깨끗한 코드는 코드 수준에서 시스템을 조정하고 확장하기 쉽게 만든다. </i></strong> 🍋
...
- `후미`: 처음부터 제대로된 시스템을 만들 수는없어 반복적으로 만들어가자는 말이 공감됨

> <strong><i>p200. 소프트웨어 시스템은 ‘수명이 짧다’는 본질로 인해 아키텍처의 점진적인 발전이 가능하다. </i></strong> 🍋🌵

> <strong><i>p210. 코드 수준에서 아키텍처 관심사를 분리할 수 있다면, 진정한 테스트 주도 아키텍처 구축이 가능해진다. 그때그때 새로운 기술을 채택해 단순한 아키텍처를 복잡한 아키텍처로 키워갈 수도 있다. </i></strong> 🍋🌵

> <strong><i>p211. 모듈을 나누고 관심사를 분리하면 지엽적인 관리와 결정이 가능해진다. </i></strong> 🍋

> <strong><i>p211. 우리는 때때로 가능한 마지막 순간까지 결정을 미루는 방법이 최선이라는 사실을 까먹곤 한다. 게으르거나 무책임해서가 아니다. 최대한 정보를 모아 최선의 결정을 내리기 위해서다. </i></strong> 🍋🌵

> <strong><i>p213. 모든 추상화 단계에서도 의도는 명확히 표현해야 한다. 그러려면 POJO를 작성하고 관점 혹은 관점과 유사한 메케니즘을 사용해 각 구현 관심사를 분리해야 한다. 시스템을 설계하든 개별 모듈을 설계하든, 실제로 돌아가는 가장 단순한 수단을 사용해야 한다는 사실을 명심하자. </i></strong> 🍋🐧🌵

- `펭귄`: EJB와 EJB의 단점, POJO가 나오게 된 계기, Spring의 탄생 배경을 찾아보면 좋을 것 같아요!
...
> <strong><i>p216. 우리들 대다수는 켄트 백이 제시한 단순한 설계 규칙 네 가지가 소프트웨어 설계 품질을 크게 높여준다고 믿는다. 켄트 벡은 다음 규칙을 따르면 설계는 '단순하다'고 말한다. 아래 목록은 중요도 순이다.</i></strong> 🌵🐧
...
- `펭귄`: 요새 가장 큰 고민이 테스트 코드 짜는 것인데, 너무 작은 단위의 기능을 테스트 하기에는 의미가 없고 너무 큰 단위의 기능을 테스트 하기에는 너무 복잡한 것 같다. 테스트가 필요한 지점을 찾기가 어려운 것 같다.
- `샐리`: A-B-C로 연결된 아키텍처가 있다고 하면, A-B의 경우가 100가지, B-C의 경우가 100가지일 때, 큰 단위(ABC)로 테스트 하는 경우는 100x100가지 경우의 수가 생기고, 작은 단위로 테스트 하는 경우(AB, BC)에는 100+100가지 경우의 수가 생길 것이다. 펭귄님 말씀처럼 어떤 단위로 테스트를 하는지, 언제 테스트를 진행하고 언제 테스트 코드를 짜야하는지에 대한 고민이 필요한 것 같다.

> <strong><i>p217. 리팩토링 단계에서 응집도를 높이고, 결합도를 낮추고, 관심사를 분리하는 등..프로그래머의 의도를 표현하고, 클래스와 메서드 수를 최소로 줄이는 단계이기도 한다.</i></strong> 🙂

- `후미`: 한번에 많은것을 하지 말고. 코드 컨벤션을 지키는 것은 좋은데, 리팩토링도 코드를 짜는 과정이니. 리팩토링에서 나머지를 생각하자.

> <strong><i>p222. 그러므로 자신의 작품을 조금 더 자랑하자. 함수와 클래스에 조금 더 시간을 투자하자. 더 나은 이름을 선택하고, 큰 함수를 작은 여럿으로 나누고, 자신의 작품에 조금만 더 주의를 기울이자.</i></strong> 🙂🐧

- `샐리`: 처음에는 오래걸리지만, 연차가 쌓일수록 더 빠르게, 견고한 코드를 짤 수 있을 것이다!
...
> <strong><i>p228. 동시성은 '때로' 성능을 높여준다. </i></strong> 🌵🙂🐧

- `후미` : [암달의 법칙](https://ko.wikipedia.org/wiki/%EC%95%94%EB%8B%AC%EC%9D%98_%EB%B2%95%EC%B9%99)
- `튜브`: 동시성이 성능을 높이기 위한 것은 맞지만 항상 그런 것은 아니다.

> <strong><i>p228. 일반적으로 동시성 버그는 재현하기 어렵다. 그래서 진짜 결함으로 간주되지 않고 일회성 문제로 여겨 무시하기 쉽다. </i></strong> 🍋🌵🙂🐧

> <strong><i>p230. 동시성 코드는 다른 코드와 분리하라. </i></strong> 🍋🌵

- `샐리`: 동시성 관련하여 테스트, 오류 검출이 어려우므로 다른 코드와 분리하라는 이 코멘트가 공감갔다.
- `튜브`: 동시성 하나만으로도 충분히 어렵다.

> <strong><i>p231. 어떤 경우에는 객체를 복사해 읽기 전용으로 사용하는 방법이 가능하다. 어떤 경우에는 각 스레드가 객체를 복사해 사용한 후 한 스레드가 해당 사본에서 결과를 가져오는 방법도 가능하다. </i></strong> 🍋

- `샐리`: 어떻게 사본이 lock을 대신할 수 있을까? / 사본을 사용하여 여러 스레드에서 동시에 처리를 한 후, 어떤 결과를 받아들일지 선택하는 방법이라는 설명이 쏙쏙 와닿았다!
- `펭귄`: 위 두 문장은 두 가지 해결책을 제시를 한 것.

> <strong><i>p232. 독자적인 스레드로, 가능하다면 다른 프로세서에서, 돌려도 괜찮도록 자료를 독립적인 단위로 분할하라. </i></strong> 🍋🌵

- `샐리`: 큰 단위의 자료를 작은 단위로 잘라서 공유자원의 공유 범위를 줄이라는 말일까? / 최대한 공유자원을 줄이라는 걸까?

> <strong><i>p232. 일부 클래스 라이브러리는 스레드에 안전하지 못한다. </i></strong> 🙂

- `후미`: 라이브러리를 맹신하지말자.. 라이브러리도 사람이 짠 코드
- `샐리`: 내가 모르게 바뀌는 라이브러리를 조심해라..

> <strong><i>p234. 처리율을 강조하면 기아 현상이 생기거나 오래된 정보가 쌓인다. 갱신을 허용하면 처리율에 영향을 미친다. </i></strong> 🍋

- `샐리`: 여기서 처리율의 의미는 한번에 처리하는 일의 크기일까? / 스터디 미팅 도중 이해해버렸다.. 짧은 태스크만 처리하다 보면, 처리율은 높겠지만 기아현상을 피할 수 없을 것!

> <strong><i>p234. 대개는 쓰기 스레드가 버퍼를 오랫동안 점유하는 바람에 여러 읽기 스레드가 버퍼를 기다리느라 처리율이 떨어진다. </i></strong> 🍋

- `샐리`: 한번도 쓰기 스레드와 읽기 스레드의 속도/처리율에 대한 생각을 못해봤던 것 같다. 쓰기/읽기 스레드가 모두 동일한 조건 내에 있을 경우, 쓰기 스레드가 더 느릴까? 쓰기가 더 느리다면 왜 더 느릴까? ANS: 쓰기를 위해서는 FAKE READ를 한번 더 거친다고 한다!

> <strong><i>p235. 주의해서 설계하지 않으면 데드락, 라이브락, 처리율 저하, 효율성 저하 등을 겪는다. </i></strong> 🍋

> <strong><i>p235. 공유 객체 하나에는 메서드 하나만 사용하라. </i></strong> 🍋🐧

- `샐리`: 하나의 공유자원은 되도록 하나의 메소드만 접근할 수 있도록 하라는 말일까? 최대한 동시성 구현할거리를 줄이라는 말일지도? 하나의 메소드만 공유자원에 접근하도록 하기.
- `펭귄`: p424를 자세히 읽어봐야 확실히 알겠지만, 메서드 사이 의존성은 우리가 어떤 코드를 짰을 때 두 메서드 사이에 의존성이 생기도록 짜는 것을 말하는 것이고, 그 의존성으로 인해 특이 케이스에서는 문제가 발생할 수 있으니 공유 객체 하나에 메서드 하나만 사용하라는 말이 나온 것 같다.

> <strong><i>p236. 코드를 짤 때는 임계영역 수를 최대한 줄여야 한다. ... 필요 이상으로 임계영역 크기를 키우면 스레드 간에 경쟁이 늘어나고 프로그램 성능이 떨어진다. </i></strong> 🍋

- `샐리`: 당연히 안전하게 임계영역의 크기를 넓혀야한다고 생각했는데, 이러한 점이 스레드간 경쟁과 같이 시스템 성능을 낮추는 원인이 될 수 있다는 점을 배워간다.

> <strong><i>p236. 영구적으로 돌아가는 시스템을 구현하는 방법과 잠시 돌다 깔끔하게 종료하는 시스템을 구현하는 방법은 다르다. </i></strong> 🍋🌵

- `튜브`: 처음부터 종료 코드를 염두해 두고 프로그램을 설계해야 한다.

> <strong><i>p237. 코드가 올바르다고 증명하기는 현실적으로 불가능하다. 테스트가 정확성을 보장하지는 않는다. </i></strong> 🍋

> <strong><i>p237. 테스트가 실패하면 원인을 추적하라. 다시 돌렸더니 통과하더라는 이유로 그냥 넘어가면 절대로 안 된다. </i></strong> 🍋

- `샐리`: 그러나 동시성 문제라면 발견과 처리가 어려운 것 같다.

> <strong><i>p238. 일회성 문제란 존재하지 않는다고 가정하는 편이 안전하다. '일회성' 문제를 계속 무시한다면 잘못된 코드 위에 코드가 계속 쌓인다. </i></strong> 🍋

- `샐리`: 일회성 오류라고 치부하며, 잘못된 코드 위에 계속 코드를 쌓지 말자. 이런 식으로 회사에서 일을 하면, 조만간 뉴스에 나오게 될 것이다.

> <strong><i>p238. 다른 플랫폼에서 돌려봐라 </i></strong> 🙂

- `후미`: os마다 메모리 정책, 스케쥴러가 다르니…

> <strong><i>p238. 스레드 환경 밖에서 생기는 버그와 스레드 환경에서 생기는 버그를 동시에 디버깅하지 마라. 먼저 스레드 환경 밖에서 코드를 올바로 돌려라. </i></strong> 🍋🌵🙂🐧

- `후미`: 멀티스레드,프로세스 환경에서 디버깅은 너무 힘들다..
- `샐리`: 일단 한 개의 스레드로 프로그램을 구성하고, 그 이후에 ‘하나의 메소드만 하나의 공유자원에 접근할 수 있다’ 등의 조건을 만족하도록 코드를 수정함이 좋을 것 같다.
- `튜브`: 일단 단일 스레드로 제대로 돌아가게 만든 후, 적절한 스레드 개수를 파악하여 멀티 스레드 구현하기

> <strong><i>p239. 스와핑을 일으키려면 프로세서 수보다 많은 스레드를 돌린다. 스와핑이 잦을수록 임계영역을 빼먹은 코드나 데드락을 일으키는 코드를 찾기 쉬워진다. </i></strong> 🍋

- `샐리`: 주워들은 이야기지만, “스와핑이 일어나는 순간부터 그 코드는 못쓴다.”라는 말이 있다.

> <strong><i>p240. 다중 스레드 코드는 플랫폼에 따라 다르게 돌아간다. 따라서 코드가 돌아갈 가능성이 있는 플랫폼 전부에서 테스트를 수행해야 마땅하다. </i></strong> 🍋

> <strong><i>p241. yield()를 삽입하면 코드가 실행되는 경로가 바뀐다. </i></strong> 🍋🙂

- `후미`: 병렬화 오픈소스 라이브러리에서 yield를 사용하던데 이런 이유일려나… 공부좀 하자
- `샐리`: 랜덤함을 부여하여 테스트 신뢰도를 높이는 방법이라니!

> <strong><i>p243. 클라이언트에게 공유 상태를 관리하는 책임을 떠넘기지 않는다. 필요하다면 객체 설계를 변경해 클라이언트에게 편의를 제공한다. </i></strong> 🍋🐧

- `샐리`: 클라이언트는 어떤 새로운 이슈를 만들어줄지 걱정될 수 있다.
...
> <strong><i>p254. 프로그래밍은 과학보다는 공예에 가깝다. 깨끗한 코드를 짜려면 지저분한 코드를 짠 뒤에 정리해야 한다는 의미다. </i></strong> 🌵🍋

- `튜브`: 프로그래밍은 정해진 방법이 있다기 보다 지저분한 코드를 점차 정리해 가는 과정이다.

> <strong><i>p264. 나는 두 인수 유형을 단계적으로 추가했다. 먼저 String 인수 유형을 추가해 다음 코드를 얻었다. </i></strong> 🍋

- `샐리`: 모든 인수를 한꺼번에 처리하려고 하는 것보다는, 쉬운 인수부터 단계적으로 처리하자.

> <strong><i>p269. 코드 구조를 유지보수하기 좋은 상태로 만들려면 지금이 적기라 판단했다. 그래서 나는 기능을 더 이상 추가하지 않기로 결정하고 리팩터링을 시작했다.</i></strong> 🐧

- `펭귄`: 리팩토링의 타이밍이 정말 중요하고 그 타이밍을 놓치면 다 뒤엎는 선택 밖에 없을 것 같다.

- `후미`: [관련 링크](https://engineering.linecorp.com/ko/blog/about-demaecan-recode-project/) “Recode는 2보 전진을 위한 1보 후퇴입니다.”라는 말이 와닿았습니다.

> <strong><i>p269. 첫째, 인수 유형에 해당하는 HashMap을 선택하기 위해 스키마 요소의 구문을 분석한다. 둘째, 명령행 인수에서 인수 유형을 분석해 진짜 유형으로 변환한다. 셋째, getXXX 메서드를 구현해 호출자에게 진짜 유형을 반환한다. </i></strong> 🌵

> <strong><i>p270. 프로그램을 망치는 가장 좋은 방법 중 하나는 개선이라는 이름 아래 구조를 크게 뒤집는 행위다. 어떤 프로그램은 그저 그런 '개선'에서 결코 회복하지 못한다. '개선'전과 똑같이 프로그램을 돌리기가 아주 어렵기 때문이다. </i></strong> 🍋🌵🐧

- `샐리`: 작은 개선에도 무너지는 코드가 좋은 코드였을까? 아무튼 나의 경우에는, 작은 개선만으로도 돌아가지 않는 경우가 많긴했다.

- `펭귄`: “개선이라는 이름 아래 구조를 크게 뒤집는 행위"가 정말 어려운 것 같다. 내 코드부터 시작해서 개선하다 보면 다른 부분도 건드려야 하는 상황이 온다. 이 행위가 필요하다면, 다른 사람들과 아에 맘먹고 시작해야 가능할 것 같다.

> <strong><i>p270. TDD는 언제 어느 때라도 시스템이 돌아가야 한다는 원칙을 따른다. 다시 말해 TDD는 시스템을 망가뜨리는 변경을 허용하지 않는다. </i></strong> 🌵🐧

- `펭귄`: 테스트코드가 정말 중요한 것 같은데, 업무에 치여 테스트코드를 못(안) 짜는 게 아쉬웠다.

> <strong><i>p272. 긴 변수 이름이 싫었다. </i></strong> 🌵

- `샐리`: 회사 코드에서 가장 힘든 것은 줄임말… 그리고 그것이 어떤 말의 약자인지 아무도 모르더라..

- `후미`: 줄임말 쓰는건 좋은데 모르는 사람이 있을 수 도 있으니.. 같은 프로젝트를 하는 사람들끼리는 공유가 필수 인것 같다.

- `튜브`: 명확한 변수 이름을 강조하던 저자도 줄임말을 쓰는구나…

> <strong><i>p274. 일단 각 인수 유형을 처리하는 코드를 모두 ArgumentMarshaler 클래스에 넣고 나서 ArgumentMarshaler 파생 클래스를 만들어 코드를 분리할 작정이었다. 그러면 프로그램 구조를 조금씩 변경하는 동안에도 시스템의 정상 동작을 유지하기 쉬워지기 때문이다. </i></strong> 🍋

> <strong><i>p320. 하지만 그렇다고 ArgsException 클래스가 오류 메시지 형식을 처리해야 옳을까? 솔직하게 말해, 이것은 절충안이다. ArgsException에게 맡겨서는 안 된다고 생각하는 독자라면 새로운 클래스가 필요하다.</i></strong> 🐧

- `펭귄`: SRP 원칙이 절대적인 원칙이라기 보다는 권장 사항인 것 같다. 당연히 지켜야 되지만 때에 따라 절충도 가능한 느낌?

> <strong><i>p321.그저 돌아가는 코드만으로는 부족하다. 돌아가는 코드가 심하게 망가지는 사례는 흔하다. 단순히 돌아가는 코드에 만족하는 프로그래머는 전문가 정신이 부족하다.</i></strong> 🐧

<small><a href="#-목차">⬆️ 위로 이동</a></small>ㅍ
...
<table>
  ...
  <tbody>
    <tr>
      ...
      <td>이 책을 읽고 나면 내 코드 한 줄에 큰 책임감을 느끼게 된다. 그리고 나쁜 코드가 얼마나 나쁜지도 깨닫게 된다. <a href="http://cleancoder.com">저자가 운영하는 사이트</a>에 강연 유튜브 링크도 있으니 같이 보면서 읽어보는 걸 추천한다. 다만 이 책은 개발 초보자를 위한 책이 절대 아니며, 어느 정도 개발이 숙련된 주니어 개발자를 위한 책이다. 또한 예시 코드가 모두 자바이기 때문에 자바와 비슷한 언어를 한 번도 써보지 않았다면 읽기 쉽지 않을 것이다. 여기서부터는 TMI인데, 저자는 "나쁜 코드에서 악취가 난다"는 표현을 자주 쓰는데 진짜 내 코드에서 냄새가 날 것 같은 기분이 든다. 그리고 저자가 쓴 책들의 제목을 보면 맨 앞 단어가 거의 항상 "Clean"이다.</td>
    </tr>
    <tr>
      <td align='center'><code>샐리</code></td>
      <td>클린코드에 대한 생각을 처음으로 하게 된 계기가 됐다. 솔직히 말하자면, 이 책 내용이 이해하기 매우 쉬운 편은 아닌 것 같다. 하지만 다른 자료(블로그나 유투브)를 참고해서라도 읽으면 도움이 될 기본적인 서적이라고 생각한다. Java Spring에 대한 내용이 예시로 들어가있어서 그런지, 후반부로 갈수록 저자가 말하고자 하는 포인트를 찾기 어려웠다. 그러나 팀원들의 생각이나 경험을 공유받으면서 많은 깨달음을 얻을 수 있었다. 현업 중, 이 책에서 배운 내용과 관련된 이야기나 코드가 나오면 꽤 재미있다.</td>
    </tr>
    <tr>
      <td align='center'><code>후미</code></td>
      <td>이 책 덕분에 단순성, 단일 책임 등 좋은 코드의 기준에 대해 배울 수 있었고, 앞으로 코드를 짜는데 있어 계속 생각 할 것 같다. 하지만 이 책을 읽으면서 몇몇 공감되지 않는 글귀들이 있었는데 이런 글귀들은 본인의 판단하에 적당히 걸러 들으면 될것 같다. 코딩 컨벤션에는 절대적인 정답은 없는것 같다. 코딩 컨벤션은 팀바팀, 회바회, 언바언(언어바이언어)등 속한 단체의 약속일 뿐인것 같다.</td>
    </tr>
    <tr>
